...
'''
создаем приложение 
мы передаем __name__, в нем содержится информация, 
...
'''
# Создадим словарь, чтобы для каждой сессии общения с навыком 
хранились подсказки, которые видел пользователь.
...
def main():
    ...
    # мы собираем словарь, который потом при помощи библиотеки
    ...
    logging.info(f'Response:  {request.json!r}')
